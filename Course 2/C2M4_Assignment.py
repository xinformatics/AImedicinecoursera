...
def hazard_ratio(case_1, case_2, cox_params):
    ...
    #print (np.exp(np.dot(cox_params,case_1))/(np.exp(np.dot(cox_params,case_2))))
    #print (np.exp(np.dot(cox_params,case_2)))
        ...
        for j in range(i+1, n):
            ...
            if not (event[i] == 0 and event[j] == 0):
                ...
                if (event[i] == 1 and event[j] == 1):
                    ...
                    if (scores[i] == scores[j]) or (y_true[i] == y_true[j]):
                        #pass
                        ties += 1
