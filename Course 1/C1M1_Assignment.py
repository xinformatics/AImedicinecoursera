...
def check_for_leakage(df1, df2, patient_col):
    ...
    #print (len(patients_in_both_groups))
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        for i in range(len(pos_weights)):
            ...
            loss += -1*K.mean((pos_weights[i]*(y_true[:,i])*(K.log(epsilon + y_pred[:,i])) + neg_weights[i]*(1-y_true[:,i])*(K.log(epsilon + 1-y_pred[:,i])))) #complete this line
        return loss
